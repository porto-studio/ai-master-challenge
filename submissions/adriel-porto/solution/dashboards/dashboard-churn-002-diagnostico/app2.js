...
const state = {
  ...
  accountFilters: {},         // { coluna: Set(valores marcados) }
...
const FILTER_PRESETS = [
  { label: "Direto + Basic", filters: { is_trial: ["False"], plan_tier: ["Basic"] } },
  { label: "Direto + Pro", filters: { is_trial: ["False"], plan_tier: ["Pro"] } },
  { label: "Direto + Enterprise", filters: { is_trial: ["False"], plan_tier: ["Enterprise"] } },
  { label: "Trial + Basic", filters: { is_trial: ["True"], plan_tier: ["Basic"] } },
  { label: "Trial + Pro", filters: { is_trial: ["True"], plan_tier: ["Pro"] } },
  { label: "Trial + Enterprise", filters: { is_trial: ["True"], plan_tier: ["Enterprise"] } },
];

const ACCOUNT_FILTER_CANDIDATES = ["industry", "country", "referral_source", "plan_tier", "is_trial", "churn_flag"];
...
function setupAccountFilters() {
  ...
  state.accountFilterColumns = [];
  if (!accounts.length) return;

  ACCOUNT_FILTER_CANDIDATES.forEach((col) => {
    ...
    if (uniques.length > 1 && uniques.length <= 20) {
      state.accountFilterColumns.push(col);
      state.accountFilters[col] = new Set(uniques);
    }
  });
...
function applyPreset(preset) {
  const accounts = state.raw.accounts || [];
  state.accountFilterColumns.forEach((col) => {
    if (preset.filters[col]) {
      state.accountFilters[col] = new Set(preset.filters[col]);
    } else {
      const uniques = Array.from(new Set(accounts.map((a) => (a[col] === undefined || a[col] === null ? "" : String(a[col])))));
      state.accountFilters[col] = new Set(uniques);
    }
...
// ---------- comparação de filtros, uma linha por vez ----------

function renderComparisonTable() {
  const box = document.getElementById("comparisonBox");
  box.hidden = state.comparisonRows.length === 0;
  let html = "<thead><tr><th>Filtro</th><th>Contas</th><th>Ativas</th><th>Canceladas</th><th>MRR atual</th><th>Valor perdido em churn</th><th>% valor perdido</th></tr></thead><tbody>";
  state.comparisonRows.forEach((r) => {
    html += `<tr><td>${r.label}</td><td>${r.total}</td><td>${r.active}</td><td>${r.canceled}</td><td>${fmtMoney(r.mrr)}</td><td class="neg">${fmtMoney(r.churnValue)}</td><td>${r.pctChurnValue.toFixed(1)}%</td></tr>`;
  });
  html += "</tbody>";
  document.getElementById("comparisonTable").innerHTML = html;
}

document.getElementById("addComparisonRow").addEventListener("click", () => {
  if (!state.derived) return;
  const label = state.lastFilterLabel || (state.singleAccountId ? `Conta ${state.singleAccountId}` : "Filtro manual");
  const cs = state.derived.currentStatus;
  const cr = state.derived.churnRank;
  state.comparisonRows.push({
    label,
    total: cs.active + cs.canceled,
    active: cs.active,
    canceled: cs.canceled,
    mrr: cs.currentMrr,
    churnValue: cr.totalChurned,
    pctChurnValue: cr.totalHistoricalRevenue ? (cr.totalChurned / cr.totalHistoricalRevenue) * 100 : 0,
  });
...
function buildAccountFilterUI() {
  ...
  state.accountFilterColumns.forEach((col) => {
    ...
    uniques.forEach((val) => {
      ...
      cb.type = "checkbox";
      cb.checked = state.accountFilters[col].has(val);
      cb.addEventListener("change", () => {
        if (cb.checked) state.accountFilters[col].add(val);
        else state.accountFilters[col].delete(val);
...
document.getElementById("resetAccountFilters").addEventListener("click", () => {
  ...
  state.lastFilterLabel = "Sem filtro (todas as contas)";
  buildAccountFilterUI();
  runAnalysis();
});
...
function getScopedAccountIds() {
  ...
  if (!state.accountFilterColumns.length) return new Set(accounts.map((a) => a.account_id));
  const ids = new Set();
  accounts.forEach((a) => {
    const pass = state.accountFilterColumns.every((col) => {
      const val = a[col] === undefined || a[col] === null ? "" : String(a[col]);
      return state.accountFilters[col].has(val);
    });
    if (pass) ids.add(a.account_id);
  });
  return ids;
}
